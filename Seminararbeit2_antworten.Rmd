...
```
#Scoping II
Funktion matrix dient eine Matrix zu bauen, als erste Position nimmt diese Funktion die Daten , hier die Zahlen von 1 bis 6, dann definiert man wieviele Spalte man will, hier 3, dann definiert man in welcher Ordnung die Daten ausgefüllt werden sollen, wenn TRUE beim byrow : sind die Zahlen in Zeilen in aufsteigende Reihenfolge gezeigt. Mit dem t(t) transponiert man die Matrix t (Transpose).
```{r}
...
???
#Dynamic Lookup
Erste Linie löscht komplett globale Umgebung, das funktioniert genauso wie rm(list = ls()). 
```{r}
